...
# from config.settings import GROQ_API_KEY3, SYNTHESIS_MODEL, SYNTHESIS_STRUCTURED_METHOD
...
from config.settings import (
    ...
    SYNTHESIS_MODEL, SYNTHESIS_STRUCTURED_METHOD,
)
from graph.state import SIFACTState, VerifiedStance
...
def _invoke_structured_with_fallback(messages: list) -> SynthesisVerdict:
    """
    ...
    for idx, api_key in enumerate(_GROQ_API_KEYS, start=1):
        try:
            logger.info("Synthesis: trying API key slot %d/%d", idx, len(_GROQ_API_KEYS))
            llm = ChatGroq(
                model=SYNTHESIS_MODEL,
            ...
            result: SynthesisVerdict = structured.invoke(messages)
            logger.info("Synthesis: succeeded with API key slot %d", idx)
            return result

        except _FALLBACK_EXCEPTIONS as exc:
            logger.warning(
                "Synthesis: API key slot %d failed (%s: %s). %s",
                idx,
                type(exc).__name__,
                exc,
...
def synthesis_node(state: SIFACTState) -> dict[str, Any]:
    ...
    try:
        out: SynthesisVerdict = _invoke_structured_with_fallback(messages)
    except Exception as exc:
        logger.exception(
            "Synthesis: all API keys exhausted (method=%s): %s",
            SYNTHESIS_STRUCTURED_METHOD,
            exc,
        )
        return {
